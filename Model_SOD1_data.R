...
path_fig4 <-"C:/Users/tquiroga/OneDrive - IBEC/Projects/From owncloud/SOD1 binding PCA/SOD1 manuscript/Plots/Figure 4 (newint)"
...
p_corr <- ggplot(data = SOD1_allmut, aes(x = abundance_score, y = binding_score)) +
  ...
  theme_classic() +
  labs(x = "Normalized abundance score", y = "Normalized binding score") +
...
ggsave(p_corr, path = path_fig4, file = "normal_correlation_apca_boca.tiff", width = 10, height = 8)

#label by stop-like mutations
SOD1_allmut <- SOD1_allmut %>% 
  mutate(stop_like = case_when(
    category_fdr_apca == "Stop-like abundance" & category_fdr_bpca == "Stop-like binding" ~ "TRUE",
    TRUE ~ "FALSE"
  ))
...
ggsave(p_corr_stop, path = path_fig4, file = "correlation_apca_boca_label_by_stoplike.tiff", width = 10, height = 8)
...
ggsave(p_loess, path = path_fig4, file = "correlation_apca_boca_label_by_residual_unfiltered.tiff", width = 10, height = 8)
...
#Save modelled data####
SOD1_subsitutions_modelled <- SOD1_allmut %>%
  filter(mutation_type %in% c("subs", "stop"))

SOD1_insertions_modelled <- SOD1_allmut %>%
  filter(mutation_type == "ins")

SOD1_deletions_modelled <- SOD1_allmut %>% 
  filter(mutation_type == "del")

#save modeled data
write.csv(SOD1_subsitutions_modelled,file="SOD1_subs_modelled.csv", row.names = FALSE)
write.csv(SOD1_insertions_modelled,file ="SOD1_ins_modelled.csv", row.names = FALSE)
write.csv(SOD1_deletions_modelled,file = "SOD1_del_modelled.csv", row.names = FALSE)
