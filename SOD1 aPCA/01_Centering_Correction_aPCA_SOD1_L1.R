...
load("SOD1_final_fitness_replicates.RData")

lib1_design <- read.delim("SOD1_lib1.tsv")
...
all_variants <- inner_join(all_variants, lib1_design, by= "aa_seq")
...
lib1_design$nt_seq <- tolower(lib1_design$nt_seq)
syn_designed <- inner_join(synonymous, lib1_design, by = "nt_seq")
...
save(all_variants_stop, file="all_variants_SOD1_stops_newfilter_aPCA.RData")
save(silent, all_variants, all_variants_stop, file="Fscore_df_SOD1_all_variants_identity_aPCA.RData")



#Save Lib1 scores table####
apca_l1 <- select(all_variants_stop, aa_seq, ID, type, count_e1_s0, count_e2_s0, count_e3_s0, count_e1_s1, count_e2_s1, count_e3_s1, input_mean_count, output_mean_count, ascore1_c, ascore2_c, ascore3_c, ascore_c, sigma, sigma_norm_iqr, low_sigma)
apca_l1$library <- 1
...
write.csv(apca_l1, file = "aPCA_L1_table.csv", row.names = FALSE)
