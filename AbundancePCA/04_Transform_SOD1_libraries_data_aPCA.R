...
load("Fscore_df_SOD1_all_variants_identity_aPCA.RData")
...
all_SOD1<-select(all_SOD1, aa_seq, starts_with('ascore'), mean_out, ID, mean_count, Pos, Mut, WT_AA, sigma_norm_iqr, type)
all_SOD1 <- rename(all_SOD1, sigma = sigma_norm_iqr)


## Load L2 data####
# First get all the variants that are trustable (low_sigma or sig_10)
load("Fscore_df_SOD1_all_variants_identity_L2_aPCA.RData")
...
all_SOD2<-select(all_SOD2, aa_seq, starts_with('ascore'), mean_out, ID, mean_count, Pos, Mut, WT_AA, sigma_norm_iqr, type)
all_SOD2 <- rename(all_SOD2, sigma = sigma_norm_iqr)

##Load L3 data####
# First get all the variants that are trustable (low_sigma or sig_10)
load("Fscore_df_SOD1_all_variants_identity_L3_aPCA.RData")
...
all_SOD3<-select(all_SOD3, aa_seq, starts_with('ascore'), mean_out, ID, mean_count, Pos, Mut, WT_AA, sigma_norm_iqr, type)
...
SOD12<-select(SOD12, WT_AA, Mut, Pos, ID,
              mean_count_1, sigma, mean_count_2, sigma_2, ascore_c.x, ascore_c.y, mean_out_1, mean_out_2, type.x, type.y)

SOD12<-rename(SOD12, ascore_1_c = ascore_c.x, ascore_2_c = ascore_c.y)
...
silentSOD123<-rbind(select(silent_SOD1, ascore_c, Mut, Region, Nmut_codons, sigma), select(silent_SOD2, ascore_c, Mut, Region, Nmut_codons, sigma), select(silent_SOD3, ascore_c, Mut, Region, Nmut_codons, sigma))
stops_SOD123<-rbind(select(stops_SOD1, ascore_c, Mut, Region), select(stops_SOD2, ascore_c, Mut, Region),select(stops_SOD3, ascore_c, Mut, Region))




dist<-rbind(silentSOD123[,c('ascore_c', 'Mut', 'Region')], stops_SOD123[,c('ascore_c', 'Mut', 'Region')])

p_hist<-ggplot(dist, aes(x=ascore_c))+
...
ggsave(p_hist, file="SOD123_overlapp_hist_identity_apca.jpg", width=8, height=4)

### Linear Regression of SOD1-SOD2 ####
# Check correlation between the centered nscores calculated in each library
corr<-cor.test(SOD12$ascore_1_c, SOD12$ascore_2_c, use="complete.obs")
R<-corr$estimate
p<-corr$p.value

# Linear Regression 
# x = SOD2 and y = SOD1 Then i can predict the values of SOD1 adjusted to SOD2
LR_SOD12<-lm(ascore_1_c~ascore_2_c, data = SOD12)
summary(LR_SOD12)
...
ggsave(p_corr, file="SOD1_SOD2_overlapp_corr_apca.jpg", width=8, height=4)

#### SOD1-SOD2 data transformation ####
##With this LR I can transform the values of SOD1
all_SOD1 <- all_SOD1 %>%
  rename(ascore_2_c = ascore_c)  

all_SOD1$ascore_2_c <- predict(LR_SOD12, newdata = all_SOD1[, c("aa_seq","ascore_2_c")])

#now SOD1 has its nscore_c.y normalized to SOD2. I will modify SOD2 to then join them
all_SOD2$ascore_1_c<-NA
...
SOD12<-rename(SOD12, mean_count_1 = mean_count.x, mean_count_2 = mean_count.y, 
                     sigma = sigma.x, sigma_2 = sigma.y, WT_AA = WT_AA.x, Mut = Mut.x, Pos = Pos.x,
                     mean_out_1 = mean_out.x, mean_out_2 = mean_out.y, 
                     ascore_1_c = ascore_2_c, ascore_2_c = ascore_c, aa_seq = aa_seq.x)

SOD12<-select(SOD12, WT_AA, Mut, Pos, ID, aa_seq, 
                     mean_count_1, sigma, mean_count_2, sigma_2, ascore_1_c, ascore_2_c, mean_out_1, mean_out_2, type.x)


SOD12<-SOD12[, c('aa_seq', 'WT_AA', 'Mut', 'Pos', 'ID', 'sigma', 'sigma_2',
                               'ascore_1_c', 'ascore_2_c', 'mean_out_1', 'mean_out_2', 'type.x')]

all_SOD2<-rename(all_SOD2, ascore_2_c=ascore_c)
all_SOD1<-all_SOD1[, c('aa_seq', 'WT_AA', 'Mut', 'Pos', 'ID', 'sigma', 'sigma_2',
                                   'ascore_1_c', 'ascore_2_c', 'mean_out_1', 'mean_out_2', 'type')]
...
SOD23<-select(SOD23, WT_AA, Mut, Pos, ID,
              mean_count_1, sigma, mean_count_2, sigma_2, ascore_c.x, ascore_c.y, mean_out_1, mean_out_2, type.x, type.y)
SOD23<-rename(SOD23, ascore_2_c = ascore_c.x, ascore_3_c = ascore_c.y)



#### Linear Regression of SOD2-SOD3 ####
corr<-cor.test(SOD23$ascore_2_c, SOD23$ascore_3_c, use="complete.obs")
R<-corr$estimate
p<-corr$p.value

# Linear Regression 
# x = SOD3 and y = SOD2 Then i can predict the values of SOD1 adjusted to SOD2
LR_SOD23<-lm(ascore_3_c~ascore_2_c, data = SOD23)
summary(LR_SOD23)
...
ggsave(p_corr, file="SOD2_SOD3_overlapp_corr_identity_apca.jpg", width=8, height=4)
...
all_SOD3$ascore_2_c <- predict(LR_SOD23, newdata = all_SOD3[, c("aa_seq","ascore_2_c")])

#now SOD1 has its nscore_c.y normalized to SOD2. I will modify SOD2 to then join them
all_SOD2$ascore_3_c<-NA
all_SOD2$sigma_2<-all_SOD2$sigma
all_SOD2$sigma<-NA
all_SOD2$mean_out_2<-all_SOD2$mean_out
all_SOD2$mean_out_1<-NA
...
SOD23<-rename(SOD23, mean_count_1 = mean_count.x, mean_count_2 = mean_count.y, 
              sigma = sigma.x, sigma_2 = sigma.y, WT_AA = WT_AA.x, Mut = Mut.x, Pos = Pos.x,
              mean_out_1 = mean_out.x, mean_out_2 = mean_out.y, 
              ascore_2_c = ascore_c, ascore_3_c = ascore_2_c, aa_seq = aa_seq.x)

SOD23<-select(SOD23, WT_AA, Mut, Pos, ID, aa_seq, 
              mean_count_1, sigma, mean_count_2, sigma_2, ascore_2_c, ascore_3_c, mean_out_1, mean_out_2, type.x)


SOD23<-SOD23[, c('aa_seq', 'WT_AA', 'Mut', 'Pos', 'ID', 'sigma', 'sigma_2',
                 'ascore_2_c', 'ascore_3_c', 'mean_out_1', 'mean_out_2', 'type.x')]

all_SOD2<-rename(all_SOD2, ascore_2_c=ascore_c)

all_SOD2<-all_SOD2[, c('aa_seq', 'WT_AA', 'Mut', 'Pos', 'ID', 'sigma', 'sigma_2',
                       'ascore_2_c', 'ascore_3_c', 'mean_out_1', 'mean_out_2', 'type')]
...
SOD_final <- SOD_final %>%
  mutate(ascore = coalesce(ascore_2_c.x, ascore_2_c.y)) %>%  
  select(-ascore_2_c.x, -ascore_2_c.y)  %>%
...
SOD_final_overlap <- SOD_final_overlap %>%
  ...
  summarise(
    across(-ascore, first),
    ascore = mean(ascore, na.rm = TRUE),
    .groups = "drop"
  )

# SOD_final_overlap <- SOD_final_overlap %>%
#   group_by(ID) %>%
#   summarise(ascore = mean(ascore, na.rm = TRUE))
...
SOD_final_df <- select(SOD_final, ID, ascore, type, sigma)


write.csv(SOD_final, file="SOD_final_dataset_aPCA_new.csv")
